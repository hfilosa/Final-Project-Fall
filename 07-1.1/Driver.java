...
public class Driver{
	...
	while (!done){
		...
		int iRow=convertRow(p1move[0]);
		int iCol=convertCol(p1move[0]);
		int fRow=convertRow(p1move[1]);
		int fCol=convertCol(p1move[1]);
		test.move(iRow,iCol,fRow,fCol);
	    }   
	}
	...
	while (!done){
		...
		int iRow=convertRow(p2move[0]);
		int iCol=convertCol(p2move[0]);
		int fRow=convertRow(p2move[1]);
		int fCol=convertCol(p2move[1]);
		test.move(iRow,iCol,fRow,fCol);
	    }    
	}
